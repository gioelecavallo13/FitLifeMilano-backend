                    ...
                    <div class="table-responsive">
                        <table class="table table-dark table-hover mb-0 align-middle">
                            ...
                            <tbody>
                                ...
                                <tr class="table-row-chat cursor-pointer" data-href="{{ route('client.courses.show', $course->id) }}?occurrence_date={{ \Carbon\Carbon::parse($course->pivot->occurrence_date)->format('Y-m-d') }}" role="button" tabindex="0">
                                    <td class="ps-4 py-3 fw-bold text-warning">{{ $course->name }}</td>
                                    <td class="py-3">{{ $course->coach ? $course->coach->first_name . ' ' . $course->coach->last_name : 'N/D' }}</td>
                                    <td class="py-3">{{ \Carbon\Carbon::parse($course->pivot->occurrence_date)->format('d/m/Y') }} @if($course->first_occurrence_date)<span class="badge {{ $course->is_repeatable ? 'bg-info' : 'bg-secondary' }}">{{ $course->is_repeatable ? 'Ripetibile' : 'Singolo' }}</span>@endif</td>
